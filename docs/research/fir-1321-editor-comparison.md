...
*   **FFmpeg.wasm** is powerful but dangerous in a Next.js/Vercel environment. It requires `SharedArrayBuffer`, which mandates `COOP` and `COEP` headers. These headers **break Stripe and Clerk Auth** if not isolated to a subdomain or handled via a specialized service worker.
