 ...
 *
 * @example
 * // In a Stripe webhook route handler:
...
export async function trackServer<T extends SEEventName>(
  ...
  // Lazy-load posthog-node to avoid bundling it into client chunks
  ...
  if (!apiKey) {
    console.warn('[PostHog/server] No API key configured — skipping server-side event:', event)
    return
  }
