...
async def test_voice_clone_and_queue(async_client):
    payload = {
        "audio_sample": "base64_encoded_audio_data",
        "name": "Test Voice"
    }
    ...
    queue_response = await async_client.get(f"/v1/queue/{clone_id}", headers=queue_headers)
...
async def test_image_generate(async_client):
    ...
    response = await async_client.post("/v1/image/generate", content=body, headers=headers)
    assert response.status_code == 200
    data = response.json()
