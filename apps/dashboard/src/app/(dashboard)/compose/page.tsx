  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <p className="text-gray-400 mb-8">Compose your video for any platform — pick a format, add a caption, and preview before publishing.</p>
          ...
          <div className="space-y-6">
            <div>
              ...
              <input type="url" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} placeholder="https://example.com/my-video.mp4"
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-sm text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              ...
              <p className="mt-2 text-xs text-gray-500">Format: <span className="text-indigo-400 font-mono">{FORMAT_LABELS[selectedPlatform.format]}</span></p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Text Overlay <span className="text-gray-500 font-normal">(optional)</span></label>
              <textarea value={textOverlay} onChange={(e) => setTextOverlay(e.target.value)} rows={3} placeholder="Add a caption or text overlay…"
                className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-sm text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none" />
            </div>
            ...
            {jobStatus === "processing" && (
              <div className="flex items-center gap-3 text-sm text-indigo-400">
                ...
                Processing…{jobId && <span className="text-gray-500 font-mono text-xs">({jobId})</span>}
              </div>
            )}
